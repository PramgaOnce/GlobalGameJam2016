...
/////// Diffuse:
                ...
                float2 node_9358 = (i.uv0+node_6070.g*float2(0.25,0));
                float4 _GradientPanner_var = tex2D(_GradientPanner,TRANSFORM_TEX(node_9358, _GradientPanner));
                float4 node_8601 = _Time + _TimeEditor;
                float node_3958 = sin((_PulseSpeed*node_8601.g));
                float3 diffuseColor = (((2.0*saturate(_GradientPanner_var.r))+_RayColor_copy.rgb)+((sin(lerp(float3(0.8,0,0),float3(0.9764706,0.1411765,0.01176471),(node_3958*0.1+0.5)))*2.0)*(node_3958*_ColorIntensity)));
                float3 diffuse = (directDiffuse + indirectDiffuse) * diffuseColor;
////// Emissive:
                float node_2748 = ((1.0 - node_9959)*(node_9959*1.0));
                float3 emissive = float3(node_2748,node_2748,node_2748);
/// Final Color:
...
/////// Diffuse:
                float NdotL = max(0.0,dot( normalDirection, lightDirection ));
                float3 directDiffuse = max( 0.0, NdotL) * attenColor;
                float4 node_9028 = _Time + _TimeEditor;
                float2 node_9358 = (i.uv0+node_9028.g*float2(0.25,0));
                float4 _GradientPanner_var = tex2D(_GradientPanner,TRANSFORM_TEX(node_9358, _GradientPanner));
                float4 node_8601 = _Time + _TimeEditor;
                float node_3958 = sin((_PulseSpeed*node_8601.g));
                float3 diffuseColor = (((2.0*saturate(_GradientPanner_var.r))+_RayColor_copy.rgb)+((sin(lerp(float3(0.8,0,0),float3(0.9764706,0.1411765,0.01176471),(node_3958*0.1+0.5)))*2.0)*(node_3958*_ColorIntensity)));
